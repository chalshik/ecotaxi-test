...
QVariantList ReportOperations::getCarsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "),\n"
        "grouped_charges AS (\n"
        "    SELECT carId, SUM(COALESCE(kwh, 0)) AS totalKwh\n"
        "    FROM charges\n"
        "    WHERE date BETWEEN '" +
        fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") +
        "'\n"
        ...
        "        FLOOR(COALESCE(cars.percentage, 0) * ((events.income * 0.9) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / 100)\n"
        "    ELSE 0 END AS ourIncome,\n"
        "    CASE WHEN ((events.income * 0.9) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) >= 0 THEN\n"
        "        FLOOR((100 - COALESCE(cars.percentage, 0)) * ((events.income * 0.9) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) / 100)\n"
...
QString ReportOperations::convertDataToHTML(const QVariantList &data) {
    ...
    for (const QString &date : groupedByDate.keys()) {
        ...
        html += "<h3>" + date + "</h3>";
        html += "<table border='1' cellpadding='5' cellspacing='0' style='margin: 0; width: 100%;'>";
        html += "<tr>";
...
void ReportOperations::createDailyPDF(QString title, QDate fromDate, QDate toDate)
{
    ...
    // Создаем PDF
    PDFmanager::createPDF2(html, title + " " + fromDate.toString("dd.MM.yyyy") + " - " + toDate.toString("dd.MM.yyyy"));
}
QVariantList ReportOperations::getAllCarsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "grouped_charges AS (\n"
        "    SELECT carId, \n"
        "           SUM(COALESCE(kwh, 0)) AS totalKwh\n"
        "    FROM charges\n"
        "    WHERE date BETWEEN '" +
        fromDate.toString("yyyy-MM-dd") + "' AND '" + toDate.toString("yyyy-MM-dd") +
        "'\n"
        ...
        "    FLOOR(SUM(kwh) * 10) AS totalKwhCost,\n"
...
QVariantList ReportOperations::getInvestorsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    SELECT\n"
        "        cars.id AS carId,\n"
        "        SUM(COALESCE(charges.kwh, 0)) * 10 AS totalKwh\n"
        "    FROM charges\n"
...
QVariantList ReportOperations::getAllInvestorsReport(QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        COALESCE(events.incomeTax, 0) AS incomeTax,\n"
        "        COALESCE(events.outcome, 0) AS outcome,\n"
        "        FLOOR((events.income * 0.9) - (COALESCE(charges.totalKwh, 0) * 10) + events.outcome) AS totalProfit,\n"
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
QVariantList ReportOperations::getDriverChargesReport( QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
QVariantList ReportOperations::getAllDriverChargesReport( QDate fromDate, QDate toDate)
{
    toDate = toDate.addDays(1);
    QVariantList result;
    dbManager &db = dbManager::getInstance();
...
QVariantList ReportOperations::getAllCarReport(int carId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        COALESCE(SUM(CASE WHEN amount > 0 THEN amount * 0.9 ELSE 0 END), 0) AS totalEventProfit\n"
        "    FROM events\n"
        "    WHERE carId = " +
        QString::number(carId) + "\n"
                                 "        AND date BETWEEN '" +
        ...
        "    SELECT \n"
        "        carId,\n"
        "        COALESCE(SUM(kwh), 0) * 10 AS totalKwh\n"
        "    FROM charges\n"
        "    WHERE carId = " +
        QString::number(carId) + "\n"
                                 "        AND date BETWEEN '" +
...
QVariantList ReportOperations::getAllDriverReport(int driverId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    SELECT driverId, SUM(COALESCE(kwh, 0)) AS totalKwh\n"
        "    FROM charges\n"
        "    WHERE driverId = " +
        QString::number(driverId) + "\n"
                                    "    AND date BETWEEN '" +
        ...
        "    COALESCE(cg.totalKwh, 0) * 10 AS totalKwh,\n"
...
QVariantList ReportOperations::getInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "    kwh,\n"
        "    outcome,\n"
        "    FLOOR(income * 0.9 - kwh + outcome) as profit,\n"
        "    percentage,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.9 - kwh + outcome) < 0) THEN \n"
        "            0 \n"
        "        ELSE \n"
        "            FLOOR((income * 0.9 - kwh + outcome) * COALESCE(percentage, 0) / 100) \n"
        "    END AS ourProfit,\n"
        "    CASE \n"
        "        WHEN (FLOOR(income * 0.9 - kwh + outcome) < 0) THEN \n"
        "            FLOOR(income * 0.9 - kwh + outcome) \n"
        "        ELSE \n"
        "            FLOOR((income * 0.9 - kwh + outcome) * COALESCE(100 - percentage, 0) / 100) \n"
        "    END AS investorsProfit\n"
        "FROM car_stats;";
...
QVariantList ReportOperations::getAllInvestorReport(int investorId, QDate fromDate, QDate toDate)
{
    ...
    QString query =
        ...
        "        COALESCE(charge_stats.kwh, 0) AS kwh,\n"
        "        COALESCE(cars.percentage, 0) AS percentage,\n"
        "        FLOOR(COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) AS profit,\n"
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) < 0 THEN 0 \n"
        "            ELSE (COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) * (COALESCE(cars.percentage, 0) / 100) \n"
        "        END) AS ourProfit,\n"
        "        FLOOR(CASE \n"
        "            WHEN (COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) < 0 THEN \n"
        "                (COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) \n"
        "            ELSE \n"
        "                (COALESCE(event_stats.income, 0) * 0.9 + COALESCE(event_stats.outcome, 0) - COALESCE(charge_stats.kwh, 0)) * ((100 - COALESCE(cars.percentage, 0)) / 100) \n"
        "        END) AS investorsProfit\n"
        "    FROM investors\n"
        "    LEFT JOIN cars ON cars.investorId = investors.id\n"
        "    LEFT JOIN event_stats ON cars.id = event_stats.carId\n"
...
QVariantList ReportOperations::getFinesReport()
{
    ...
    QString query =
        ...
        "   fines.id,\n"
        "   fines.date AS date,\n"
        "   CASE WHEN fines.carId = 0 THEN '-' ELSE CASE WHEN cars.id IS NULL THEN 'удален' ELSE cars.sid END END AS carId,\n"
